...
Plain dict owned by `EarlyStopEngine` (`storage/early_stop.py`) and injected via `StageResources.early_stop` — the same lifecycle pattern as `gather_skip`/`enrichment`, rather than a literal SearchStage attribute (implementation note, September 2026): key → {deque of window hashes with in-window dedupe, pages_fetched, max_pages, first_stop_page, novel-counted set}. Bounded by active partitions × W. Cross-run persistence unnecessary: frontiers are stable because RefineEngine regenerates identical query strings and the registry carries the memory.
...
`would_stop_at` records append to `<workspace>/registry_decisions.jsonl` with `type='early_stop'` (partition query, page, window stats, ratio). `novel_after_stop`: the distinct links that the amended gather-skip conjunction would not skip (still require research) and that arrived after the hypothetical stop page for their partition. This conservatively over-counts vs. a `first_seen_ts`-only join — it measures the work a stop would actually miss — which is the safe direction for a promotion gate. Promotion gate (task): `novel_after_stop ≈ 0` across ≥3 diverse staging query sets before default-on is even discussed.

# Delta Spec: repo-meta-enrichment
...
Every enrichment need SHALL consult the cache first; entries fresh within TTL SHALL be served with zero network requests. Fetches trigger lazily at two points: (a) AcquisitionStage encountering a repository that is uncached or stale; (b) the gather-skip evaluator encountering stale cache for candidate links (wired only after add-gather-skip is implemented). Duplicate encounters of the same repository within a batch, page, or run SHALL coalesce into a single fetch.
